...
public class ViewSubmissionImmediateResolver {
    ...
    public ViewSubmissionImmediateDto resolve(JsonNode payload) {
    ...
    private ViewSubmissionImmediateDto resolveDefaultSubmit(JsonNode payload) {
        ...
        if (OPTION_CUSTOM.equals(selected)) {
            ...
            return ViewSubmissionImmediateDto.noEnqueue(response);
        }
        if (OPTION_NOW.equals(selected)) {
            return ViewSubmissionImmediateDto.enqueue(SlackActionResponse.clear());
        }

        return parseMinutes(selected)
                .map(ignored -> ViewSubmissionImmediateDto.enqueue(SlackActionResponse.clear()))
                .orElseGet(() -> ViewSubmissionImmediateDto.noEnqueue(
                        SlackActionResponse.errors(Map.of("time_block", INVALID_TIME_MESSAGE))
                ));
    }

    private ViewSubmissionImmediateDto resolveCustomSubmit(JsonNode payload) {
        ...
        if (!errors.isEmpty()) {
            return ViewSubmissionImmediateDto.noEnqueue(SlackActionResponse.errors(errors));
        }
