...
import com.slack.bot.application.interactivity.box.aop.aspect.exception.ViewSubmissionAopProceedException;
...
public class ViewSubmissionInboxEnqueueAspect {
    ...
    public Object enqueue(ProceedingJoinPoint joinPoint, JsonNode payload) {
        ...
        ViewSubmissionImmediateDto syncResultDto = proceedInViewSubmissionContext(joinPoint);
    ...
    private ViewSubmissionImmediateDto proceedInViewSubmissionContext(ProceedingJoinPoint joinPoint) {
        try {
    ...
    private ViewSubmissionImmediateDto validateSyncResult(Object result) {
        if (!(result instanceof ViewSubmissionImmediateDto syncResultDto)) {
            throw new IllegalStateException(
                    "view_submission enqueue AOP 대상 메서드는 ViewSubmissionImmediateDto를 반환해야 합니다."
            );
        }
