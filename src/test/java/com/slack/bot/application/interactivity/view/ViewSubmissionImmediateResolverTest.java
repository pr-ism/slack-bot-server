...
class ViewSubmissionImmediateResolverTest {
    ...
    ViewSubmissionImmediateResolver resolver;

    @BeforeEach
    void setUp() {
        Clock fixedClock = Clock.fixed(Instant.parse("2026-02-15T00:00:00Z"), ZoneId.of("Asia/Seoul"));

        resolver = new ViewSubmissionImmediateResolver(
                fixedClock,
                reviewTimeValidator,
    ...
    void 알_수_없는_callback이면_빈_응답을_반환하고_enqueue하지_않는다() {
        ...
        // when
        ViewSubmissionImmediateDto actual = resolver.resolve(payload);
