...
class SlackNotificationOutboxStatusTest {

    @ParameterizedTest
    @MethodSource("completedStatusArguments")
    void 완료_status는_validateHistoryStatus를_통과한다(SlackNotificationOutboxStatus status) {
        // when & then
    ...
    void 미완료_status는_validateHistoryStatus에서_예외를_던진다(SlackNotificationOutboxStatus status) {
        // when & then
    ...
    void SENT는_absent_failure만_허용한다(
            BoxFailureSnapshot<SlackInteractionFailureType> failure,
            boolean shouldPass
    ) {
        // when & then
        if (shouldPass) {
            assertDoesNotThrow(() -> SlackNotificationOutboxStatus.SENT.validateHistoryFailure(failure));
            return;
        }

        assertThatThrownBy(() -> SlackNotificationOutboxStatus.SENT.validateHistoryFailure(failure))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("SENT history에는 실패 정보가 없어야 합니다.");
    }
    ...
    void RETRY_PENDING는_허용된_failureType만_통과한다(
        ...
        // when & then
        if (shouldPass) {
            assertDoesNotThrow(() -> SlackNotificationOutboxStatus.RETRY_PENDING.validateHistoryFailure(failure));
            return;
        }

        assertThatThrownBy(() -> SlackNotificationOutboxStatus.RETRY_PENDING.validateHistoryFailure(failure))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void FAILED는_허용된_failureType만_통과한다(
        ...
        // when & then
        if (shouldPass) {
            assertDoesNotThrow(() -> SlackNotificationOutboxStatus.FAILED.validateHistoryFailure(failure));
            return;
        }

        assertThatThrownBy(() -> SlackNotificationOutboxStatus.FAILED.validateHistoryFailure(failure))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(expectedMessage);
    }
    ...
    private static Stream<Arguments> completedStatusArguments() {
        return Stream.of(
                Arguments.of(SlackNotificationOutboxStatus.RETRY_PENDING),
                Arguments.of(SlackNotificationOutboxStatus.SENT),
                Arguments.of(SlackNotificationOutboxStatus.FAILED)
        );
    }

    private static Stream<Arguments> notCompletedStatusArguments() {
        return Stream.of(
                Arguments.of(SlackNotificationOutboxStatus.PENDING),
                Arguments.of(SlackNotificationOutboxStatus.PROCESSING)
        );
    }
