SET REFERENTIAL_INTEGRITY FALSE;

TRUNCATE TABLE review_reminders;
TRUNCATE TABLE slack_interaction_inbox_history;
TRUNCATE TABLE slack_interaction_inbox;
TRUNCATE TABLE slack_notification_outbox_history;
TRUNCATE TABLE slack_notification_outbox;
TRUNCATE TABLE review_notification_outbox_history;
TRUNCATE TABLE review_notification_outbox;
TRUNCATE TABLE review_request_inbox_history;
TRUNCATE TABLE review_request_inbox;
TRUNCATE TABLE round_reviewer;
TRUNCATE TABLE pull_request_rounds;
TRUNCATE TABLE review_reservation_interactions;
TRUNCATE TABLE review_reservations;
TRUNCATE TABLE workspaces;
TRUNCATE TABLE oauth_verification_states;
TRUNCATE TABLE project_members;
TRUNCATE TABLE channels;
TRUNCATE TABLE access_links;
TRUNCATE TABLE access_link_sequences;
TRUNCATE TABLE notification_settings;
TRUNCATE TABLE projects;

SET REFERENTIAL_INTEGRITY TRUE;
